...
├── serve.sh             # 启动 API 服务
├── score.sh             # 命令行交互式打分
└── score_test.js        # API 接口测试脚本（Node.js, 100 条用例）
```
...
> 以下数据来自 `score_test.js` 在 100 条测试用例上的实测结果（5 个难度等级，每级 20 题）。
...
|------|--------|----------|---------|---------|----------|
| 1 (极简) | 1-2 | 10% | 25% | 65% | 3.60 |
| 2 (简单) | 3-4 | 15% | 65% | 95% | 4.80 |
| 3 (中等) | 5-6 | 40% | 100% | 100% | 5.60 |
| 4 (困难) | 7-8 | 30% | 90% | 100% | 6.95 |
| 5 (极难) | 9-10 | 0% | 20% | 75% | 7.45 |

### 路由分布

- **flash**（18 次）：均分 3.61，范围 2-5
- **pro**（82 次）：均分 6.13，范围 3-8
